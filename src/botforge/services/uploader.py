...
class UpstashVectorError(Exception):
    """Base exception for Upstash Vector operations"""
    pass


class EmbeddingError(UpstashVectorError):
    """Exception for embedding generation failures"""
    pass


class FileProcessingError(UpstashVectorError):
    """Exception for file processing failures"""
    pass


class ChunkingError(UpstashVectorError):
    """Exception for text chunking failures"""
    pass


class VectorUploadError(UpstashVectorError):
    """Exception for vector upload failures"""
    pass


class QueryError(UpstashVectorError):
...
class UpstashVectorUploader:
    def __init__(self, chunk_size: int = 1000, chunk_overlap: int = 100):
        ...
        self.logger.warning("PyMuPDF not available. Using PyPDF2 only. Install with: pip install PyMuPDF")
        ...
        try:
            ...
            raise UpstashVectorError(f"Failed to load sentence transformer model: {e}")
        ...
        try:
            ...
            self.logger.info("Upstash Vector index initialized successfully")
        except Exception as e:
            self.logger.error(f"Error initializing Upstash Vector index: {e}")
            raise UpstashVectorError(f"Failed to initialize Upstash Vector index: {e}")
        ...
        self.logger.info(f"Chunking config: size={self.chunk_size}, overlap={self.chunk_overlap}")

    def get_files(self, user_id: str, bot_id: str) -> List[Path]:
        """Get all supported files for a user/bot combination"""
        ...
        try:
            ...
            raise FileProcessingError(f"Failed to scan directory {base_path}: {e}")
    ...
    def extract_pdf_text_pymupdf(self, file_path: Path) -> str:
        ...
        try:
            ...
            raise FileProcessingError(f"PyMuPDF extraction failed for {file_path.name}: {e}")

    def extract_pdf_text_pypdf2(self, file_path: Path) -> str:
        """Extract text from PDF using PyPDF2 - fallback method"""
        try:
            self.logger.info(f"Using PyPDF2 to extract text from: {file_path.name}")
            ...
            raise FileProcessingError(f"PyPDF2 extraction failed for {file_path.name}: {e}")
    ...
    def load_text(self, file_path: Path) -> str:
        ...
        try:
            self.logger.info(f"Loading text from: {file_path.name}")
            
            if file_path.suffix.lower() == ".txt":
                text = file_path.read_text(encoding="utf-8").strip()
                self.logger.debug(f"Loaded {len(text)} characters from TXT file")
                return text
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    self.logger.debug(f"Loaded {len(text)} characters from JSON file")
                    return text
                ...
                if self.pymupdf_available:
                    try:
                        ...
                        self.logger.warning(f"PyMuPDF failed for {file_path.name}, trying PyPDF2")
                ...
                if not text:
                    raise FileProcessingError(f"No text could be extracted from PDF: {file_path.name}")
                
                self.logger.info(f"Extracted {len(text)} characters from PDF: {file_path.name}")
                return text
            else:
            ...
            raise FileProcessingError(f"Failed to read file {file_path.name}: {e}")
    ...
    def chunk_text(self, text: str, chunk_size: int = None, chunk_overlap: int = None) -> List[str]:
        ...
        try:
            ...
            self.logger.info(f"Created {len(final_chunks)} semantic chunks from {len(text)} characters")
            
            # Log chunk size distribution for debugging
            sizes = [len(chunk) for chunk in final_chunks]
            if sizes:
                self.logger.debug(f"Chunk sizes - Min: {min(sizes)}, Max: {max(sizes)}, Avg: {sum(sizes)//len(sizes)}")
            
            return final_chunks
            
        except Exception as e:
            self.logger.error(f"Error during text chunking: {e}")
    ...
    def _split_by_sections(self, text: str) -> List[str]:
        ...
        try:
            ...
            self.logger.warning(f"Error in section splitting, falling back to simple split: {e}")
            return [text]
    ...
    def _split_section_intelligently(self, text: str, chunk_size: int, chunk_overlap: int) -> List[str]:
        ...
        try:
            ...
            self.logger.warning(f"Error in intelligent splitting, falling back to simple chunking: {e}")
    ...
    def _split_into_sentences(self, text: str) -> List[str]:
        ...
        try:
            ...
            self.logger.warning(f"Error in sentence splitting: {e}")
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                try:
                    ...
                    if not vector:
                        self.logger.warning(f"Empty vector for chunk {i}")
                        continue
                    ...
                    self.logger.debug(f"Prepared chunk {i+1}/{len(chunks)} (ID: {doc_id}, Length: {len(chunk)})")
            ...
            if vectors_to_upsert:
                try:
                    self.logger.info(f"Upserting {len(vectors_to_upsert)} vectors to namespace: {namespace}")
                    ...
                    self.logger.info(f"Successfully uploaded: {file_path.name} ({uploaded_chunks} chunks)")
                    
                    return {"uploaded": uploaded_chunks, "skipped": 0}
                    
                except Exception as e:
                    self.logger.error(f"Failed batch upsert for {file_path.name}: {e}")
                    raise VectorUploadError(f"Failed to upload vectors for {file_path.name}: {e}")
            else:
                self.logger.warning(f"No vectors to upsert for {file_path.name}")
                return {"uploaded": 0, "skipped": 1}
            
        except (FileProcessingError, ChunkingError, EmbeddingError, VectorUploadError):
            raise
        except Exception as e:
            self.logger.error(f"Failed to upload {file_path.name}: {e}")
            raise VectorUploadError(f"Failed to upload file {file_path.name}: {e}")
    ...
    def upload_user_bot_data(self, user_id: str, bot_id: str) -> Dict[str, Any]:
        ...
        self.logger.info(f"Starting upload for user: {user_id}, bot: {bot_id}")
        ...
        try:
            ...
            for file in files:
                try:
                    self.logger.info(f"Processing file {files_processed + 1}/{len(files)}: {file.name}")
            ...
            self.logger.info(f"Upload complete for {namespace}: {files_processed} files processed, "
                           f"{total_uploaded} chunks uploaded, {total_skipped} chunks skipped")
            ...
            self.logger.error(f"Upload failed for {user_id}/{bot_id}: {e}")
            raise VectorUploadError(f"Failed to upload data for {user_id}/{bot_id}: {e}")
    ...
    def query_user_bot_data(self, query: str, user_id: str, bot_id: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise QueryError(f"Failed to query namespace {namespace}: {e}")
    ...
    def inspect_stored_data(self, user_id: str, bot_id: str, num_samples: int = 3) -> List[Dict[str, Any]]:
        ...
        try:
            namespace = self.create_namespace(user_id, bot_id)
            self.logger.info(f"Inspecting stored data in namespace: {namespace}")
            ...
            self.logger.info(f"Inspecting {len(results)} sample records")
            return results
            
        except Exception as e:
            self.logger.error(f"Inspection failed for namespace {namespace}: {e}")
            raise QueryError(f"Failed to inspect data for {user_id}/{bot_id}: {e}")

    def debug_single_record(self, user_id: str, bot_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if results:
                result = results[0]
                self.logger.debug(f"DEBUG: Raw result structure keys: {list(result.keys())}")
                
                metadata = result.get('metadata', {})
                self.logger.debug(f"Keys in metadata: {list(metadata.keys())}")
                
                # Check what text fields exist
                for key in ['content', 'text', 'data']:
                    if key in metadata:
                        self.logger.debug(f"Found text in '{key}': {len(metadata[key])} chars")
                
                return result
            else:
                self.logger.warning("No results found for debugging")
                return None
                
        except Exception as e:
            self.logger.error(f"Debug failed for namespace {namespace}: {e}")
            raise QueryError(f"Failed to debug data for {user_id}/{bot_id}: {e}")
    ...
    def delete_user_bot_data(self, user_id: str, bot_id: str) -> bool:
        ...
        try:
            namespace = self.create_namespace(user_id, bot_id)
            self.logger.info(f"Deleting all data for namespace: {namespace}")
            ...
            raise VectorUploadError(f"Failed to delete data for {user_id}/{bot_id}: {e}")

    def test_connection(self) -> bool:
        """Test the connection to Upstash Vector"""
        try:
            self.logger.info("Testing Upstash Vector connection...")
            ...
            raise UpstashVectorError(f"Connection test failed: {e}")
